...
5.  Скачайте готовый или соберите самостоятельно `parser.exe` (прикреплю позже).

6.  В корневой папке Flutter-приложения (`invoice_helper`) создайте директорию `backend` и поместите в неё `parser.exe`.
...
8.  Готово! Запускайте собранный `invoice_helper.exe` (по умолчанию из папки `build\windows\runner\Release`).
