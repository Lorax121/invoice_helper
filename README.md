...
2.  В случае успеха, вы сможете открыть **дополнительное окно-оверлей** (поверх всех других окон) с удобным интерфейсом для копирования названий и цен. В нём также можно указать наценку/НДС/... .
...
7.  В корневой папке Flutter-приложения (`invoice_helper`) создайте директорию `backend` и поместите в неё `parser.exe`.

8.  Скачайте и установите **Tesseract-OCR** (например, [отсюда](https://docs.coro.net/featured/agent/install-tesseract-windows/)). Корневую папку `Tesseract-OCR` переименуйте в `tesseract` и поместите её также в папку `backend`.
