...
**Пример случая, когда удобно использовать:** ваши поставщики прислали вам товарные накладные, которые вы должны вручную обработать (перенести товары из накладных в 1C предприятие), если раньше вы перепечатывали сотни товаров для поиска в номенклатуре по названию, то сейчас, с помощью этого приложения, процесс должен быть проще и быстрей - вы  загружаете все накладные (лучше всего если поставщик присылает вам текстовые PDF) в это приложение и с помощью мыши и **`Ctrl + V`** (без набора текста вручную) переносите товары из накладных в вашу программу 1С.

<p align="center">
  <img src="images/app_interface/main-interface-screenshot-v1.1.0.jpg" width="400"/>
  <img src="images/app_interface/overlay-interface-screenshot-v1.1.0.jpg" width="250"/>
</p>
...
1.  **Передайте в приложение ваши PDF-файлы или сканы**, содержащие накладные. Программа попытается найти в них таблицы с товарами (ищем только таблицы, содержащие строку нумерации столбцов 1,2,3...).
2.  В случае успеха, вы сможете перейти в режим **оверлея** (поверх всех других окон) с удобным интерфейсом для копирования названий, цен и количества (опционально). В нём также можно указать наценку/НДС/... .
3.  В режиме оверлея выберите нужный товар и **кликните на слова**, которые хотите скопировать (порядок кликов учитывается), или скопируйте название целиком.
4.  Выбранный текст попадёт в буфер обмена. Перейдите в программу 1С, кликните на поле ввода названия и нажмите **`Ctrl + V`**.
5. Чтобы вернуться на главную страницу приложения просто закройте оверлей.
...
* Особенность бекенда: Ядро `dedoc`: таблицы неплохо определяются при наличии текстового слоя в pdf, но со сканами и фото всё кратно хуже - при плохом качестве скана/фото таблица может не найтись или текст может распознаться плохо. 
Ядро `camelot` - поддерживает только PDF файлы с текстовым слоем, но работает значительно быстрей `dedoc`.
Бекенд легко заменить на что то другое, буду благодарен если ваши светлые умы согут прикрутить что то более легковесное и точное (в особенности для сканов/фото).
...
5.  Скачайте готовый или соберите самостоятельно `parser.exe`.
    ...
    * ```bash
        ...
        Готово, файл `parser.exe` и папку `_internal` ищем по пути `backend\dist\parser`.
    
6.  Поместите `parser.exe` и папку `_internal` в папку `frontend\build\windows\runner\Release\backend` (создайте её).
